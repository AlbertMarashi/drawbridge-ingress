...
async fn main() {
    ...
    routing_table.subscribe(Box::new(move || {
    ...
    let https_server = Server::builder(TlsAcceptor::new(tls_cfg, https_incoming))
        .serve(proxy_service_https);
...
async fn proxy_request (rt: Arc<RoutingTable>, req: Request<Body>, remote_addr: SocketAddr, lets_encrypt: Arc<LetsEncrypt>) -> Result<Response<Body>, !> {
