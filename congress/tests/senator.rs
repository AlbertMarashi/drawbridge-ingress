...
use congress::{Senator, Peer, RPCNetwork, Request, RPC};
use futures::join;
...
async fn can_create_rpc_network() {
    ...
    let a: Arc<SenatorType> = Senator::new(Duration::from_millis(1000), rpc_a); // peer 1
    let b: Arc<SenatorType> = Senator::new(Duration::from_millis(1000), rpc_b); // peer 2
...
async fn can_push_custom_messages(){
    ...
    b.on_message(move |msg| {
        match msg {
            Message {
                msg: MessageType::Request(MyReq::A),
                .
            } => tx.send(()).unwrap(),
            _ => (),
        }
...
async fn deleting_a_peer_works() {
    ...
    let a: Arc<SenatorType> = Senator::new(Duration::from_millis(1000), rpc_a.clone()); // peer 1
    let b: Arc<SenatorType> = Senator::new(Duration::from_millis(1000), rpc_b.clone()); // peer 2
